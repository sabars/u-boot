...
#ifndef __CONFIG_ATS3605_COMMON_H__
#define __CONFIG_ATS3605_COMMON_H__
...
#define CONFIG_SYS_INIT_SP_ADDR		(CONFIG_SYS_SDRAM_BASE + 0xdff000)

/* Some commands use this as the default load address */
#define CONFIG_SYS_LOAD_ADDR		(CONFIG_SYS_SDRAM_BASE + 0x7ffc0)

/* Generic Interrupt Controller Definitions */
#define GICD_BASE			(0xb00f1000)
#define GICC_BASE			(0xb00f2000)
...
#define CONFIG_EXTRA_ENV_SETTINGS_COMMON				\
	...
	"ramdisk_addr_r=0x10fffc0\0"					\
	"no_ramdisk=-\0"					\
	"fdt_addr_r=0xe00000\0"					\
	"fdt_high=0xffffffff\0"					\
	"initrd_high=0xffffffff\0"				\
	"scriptaddr=0x1000000\0" \
	"bootenv=uEnv.txt\0" \
	"bootscr=boot.scr\0" \
	"splashimage=0x3000000\0" \
	"splashpos=m,m\0" \
	...
	"norargs=setenv devtype nor; setenv devpart ${bootdisk}:${bootpart}\0" \
	"mboot=run loadkernel; run loadramdisk; run loadfdt;" \
		"bootm ${kernel_addr_r} ${ramdisk_addr_r} ${fdt_addr_r}\0"	\
	"mnorboot=run loadkernel; run loadfdt;" \
		"bootm ${kernel_addr_r} ${no_ramdisk} ${fdt_addr_r}\0"\
	"setbootenv=" \
	...
	"mmcboot=echo boot from mmc card ...; "		\
		"run mmcargs; run setbootenv; run mboot\0"				\
	"emmcboot=echo boot from emmc card ...; "		\
		"run emmcargs; run setbootenv; run mboot\0"				\
	"nandboot=echo boot from nand card ...; "	\
		"run nandargs; run setbootenv; run mboot\0"		\
	"norboot=echo boot from nor card ...; "	\
		"run norargs; run setbootenv; run mnorboot\0"		\
...
#define SET_ENTER_BOOT_CMDLINE      "1"

#define CONFIG_ATS3605_BOOTPARA
#define CONFIG_BOARD_LATE_INIT


#endif /* __CONFIG_ATS3605_COMMON_H__ */

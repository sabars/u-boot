...
* @btype: msc_system_info type
* @device_id: ADFUD' stands for "Actions firmware update device";
...
static int check_adfu_command(struct fsg_common *common,
			enum data_direction data_dir, const char *name)
{
	printf("ADFU command: %s; LUN=%d, Dc=%d;  Hc=%d\n", name, common
...
static int do_usb_adfu_switch_to_adfu(struct fsg_common *common,
			struct fsg_buffhd *bh)
{
	u8 *buf;
...
static int do_usb_adfu_status_inquiry(struct fsg_common *common,
			struct fsg_buffhd *bh)
{
	u8 *buf = (u8 *)bh->buf;
	...
	printf("msc_system_info blength 0x%X\n", s_status.blength);
...
static int do_adfu_cmd(struct fsg_common *common,
			struct fsg_buffhd *bh, u8 adfu_command)
{
	int reply = -EINVAL;
	...
	case SC_USB_ADFU_SWITCH_TO_ADFU:
		...
		reply = check_adfu_command(common,
					DATA_DIR_TO_HOST, "ADFU_0XCB");
		if (reply == 0)
			reply = do_usb_adfu_switch_to_adfu(common, bh);
		else
			printf("0xcb check_command ERROR 0x%x\n", reply);
		break;
	case SC_USB_ADFU_STATUS_INQUIRY:
		restart_flag = 0;
		common->data_size_from_cmnd = get_le32(&common->cmnd[7]);
		reply = check_adfu_command(common,
					DATA_DIR_TO_HOST, "ADFU_0XCC");
		if (reply == 0)
			reply = do_usb_adfu_status_inquiry(common, bh);
		else
			printf("0xcc check_command ERROR 0x%x\n", reply);
		break;
	default:
		break;
	}
				...
				struct fsg_buffhd *bh)
{
	...
	ret = write_misc_info(buf, serial_len);
...
static int do_actions_get_miscinfo_cmd(struct fsg_common *common,
				struct fsg_buffhd *bh)
{
	u8 *buf = (u8 *)bh->buf;
	int ret;
...
DECLARE_GADGET_BIND_CALLBACK(usb_dnl_adfu, fsg_add);
